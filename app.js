...
let backend = (localStorage.getItem('aither_backend_url') || DEFAULT_BACKEND).replace(/\/$/, '');
let cache = {};
...
function setConnection(state,text){ const p=$('connectionPill'); p.textContent=text; p.className='pill '+state; }
function apiUrl(path){ return backend + path; }
async function api(path, options={}){
  if(!backend) throw new Error('Backend URL is not configured.');
  const response=await fetch(apiUrl(path),{...options,credentials:'include',headers:{Accept:'application/json','Content-Type':'application/json',...(options.headers||{})}});
  let data={}; try{ data=await response.json(); }catch{}
  if(!response.ok) throw new Error(data.detail || `Request failed (${response.status})`);
  return data;
}
async function checkBackend(){
  setConnection('muted','Connecting…');
  const started=performance.now();
  try{
    const health=await api('/api/health');
    const ms=Math.round(performance.now()-started);
    cache.health=health;
    setConnection('ok',`Connected · ${ms}ms`);
    render();
    return true;
  }catch(e){
    setConnection('bad','Backend offline');
    return false;
  }
}
async function loadData(silent=false){
  ...
  try{
    ...
    const connected=[status,health,version,config].some(x=>!x.error);
    if(connected){
      const ms=health?.timestamp ? '' : '';
      setConnection('ok',`Connected to Render${ms}`);
    }else setConnection('bad','Backend offline');
    render();
...
function render(){
  ...
  $('profileCard').innerHTML=user?`<div class="profile-avatar">${esc(user.name).charAt(0).toUpperCase()}</div><div><strong>${esc(user.name)}</strong><span>${esc(user.email)}</span></div>`:`<div class="profile-avatar">?</div><div><strong>Not signed in</strong><span>Sign in through an Aither app to use your shared account.</span></div>`;
